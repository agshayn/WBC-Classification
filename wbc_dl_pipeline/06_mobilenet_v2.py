##### MobileNetV2 (no oversampling/2-stage/TTA)
import os
import time
import copy
import numpy as np
...
def create_model(num_classes=NUM_CLASSES, pretrained=True, freeze_backbone=False):
...
def train_model(train_loader, val_loader, weight_tensor, device,
    ...
    model = create_model(NUM_CLASSES, pretrained=True, freeze_backbone=freeze_backbone)
...
@torch.no_grad()
def predict_test(model, test_df, idx2label, device):
...
def save_submission(ids, labels, label_col='label', id_col='id'):
    os.makedirs(SUBMISSIONS_DIR, exist_ok=True)
...
if __name__ == '__main__':
    ...
    test_model = create_model(NUM_CLASSES, pretrained=True)
    ...
    plot_history(history, save_path=f"{MODEL_NAME}_curves.png")
    ...
    test_ids, test_labels_pred = predict_test(model, test_df, idx2label, device)
    sub = save_submission(
        ids=test_ids,
