# %% [markdown]
# # 🏆 ConvNeXt-Small — Config V4 (Oversampling + 2-Stage)
# CNN moderne (2022) qui rivalise avec les Vision Transformers.
...
print(f"✅ ConvNeXt-Small V4 : IMG={IMG_SIZE}, BS={BATCH_SIZE}")
...
def make_model(nc=13,pt=True):
    m=models.convnext_small(weights='IMAGENET1K_V1' if pt else None)
...
print(f"✅ ConvNeXt-Small: {sum(p.numel() for p in model.parameters())/1e6:.1f}M params")

# %% Train utils
def freeze(m,f=True):
    for n,p in m.named_parameters():
        if 'classifier' not in n: p.requires_grad=not f
    print(f"   {'🔒' if f else '🔓'} Trainable: {sum(p.numel() for p in m.parameters() if p.requires_grad)/1e6:.1f}M")

def mk_opt(m,lrb,lrh):
    b,h=[],[]
    for n,p in m.named_parameters():
        if p.requires_grad: (h if 'classifier' in n else b).append(p)
    g=[{'params':h,'lr':lrh}]
...
model,f1s1,_,_=stage(model,train_ld,val_ld,crit,device,sc,S1_EPOCHS,S1_LR_BB,S1_LR_HEAD,S1_PAT,"📗 STAGE1: ConvNeXt",S1_FREEZE)
torch.save({'model_state_dict':model.state_dict(),'val_f1':f1s1,'model_name':'convnext_small'},'checkpoints/best_convnext_v4_stage1.pth')
...
model,f1s2,vp,vl=stage(model,s2ld,val_ld,s2cr,device,sc,S2_EPOCHS,S2_LR,S2_LR*2,S2_PAT,"📘 STAGE2: ConvNeXt rares",0,f1s1)
torch.save({'model_state_dict':model.state_dict(),'val_f1':max(f1s1,f1s2),'model_name':'convnext_small'},'checkpoints/best_convnext_v4_final.pth')
print(f"\n📊 ConvNeXt F1: S1={f1s1:.4f} S2={f1s2:.4f} Best={max(f1s1,f1s2):.4f}")
...
sub.to_csv("submissions/submission_convnext_v4_tta.csv",index=False)
print(f"✅ submissions/submission_convnext_v4_tta.csv")
